...
resource "aws_security_group" "load_balancer_sg_1" {
  name        = "allow_http"
...
resource "aws_lb" "lb_1" {
  ...
  security_groups   = [aws_security_group.load_balancer_sg_1.id]
  subnets            = [aws_subnet.main_vpc-subnet_1.id,aws_subnet.main_vpc-subnet_2.id]
